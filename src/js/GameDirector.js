	...
	GameDirector.prototype.check = function() {
		...
		setInterval(function() {
			...
			for(var i = oSelf.enemyArr.length - 1; i >= 0; i--) {
				...
				//判断玩家飞机和敌机是否放生了碰撞
				if(oSelf.isCrash(enemy.ele, oSelf.player.ele) && oSelf.player.isCrashFlag == false && enemy.isCrashFlag == false) {
				...
				for(var j = oSelf.player.bulletArr.length - 1; j >= 0; j--) {
					...
					if(oSelf.isCrash(bullet.ele, enemy.ele) && bullet.isCrashFlag == false && enemy.isCrashFlag == false) {
